...
print("port to modern MAME lua engine by zero 2020")

--------------------------------------------------------------------------------
-- bit bit.band implement
do
...
local function to_bits(n)
 ...
 local cnt = 1
 while (n > 0) do
  local last = n%2 -- local last = math.mod(n,2)
  if(last == 1) then
   tbl[cnt] = 1
  else
   tbl[cnt] = 0
...
local function tbl_to_number(tbl)
 local n = #tbl -- local n = table.getn(tbl)
...
local function expand(tbl_m, tbl_n)
 local big = {}
 local small = {}
 if(#tbl_m > #tbl_n) then  -- if(table.getn(tbl_m) > table.getn(tbl_n)) then
  big = tbl_m
  small = tbl_n
 else
  big = tbl_n
 ...
 -- expand small
 for i = #small + 1, #big do  -- for i = table.getn(small) + 1, table.getn(big) do
  small[i] = 0
 end
...
local function bit_and(m, n)
 ...
 local tbl = {}
 local rslt = math.max(#tbl_m, #tbl_n) -- local rslt = math.max(table.getn(tbl_m), table.getn(tbl_n))
 for i = 1, rslt do
...
-- local bit = {}
-- bit.band = function(a,b) return a and b end
...
local profile = {
	...
	{	games = {"sf2ce","sf2hf","sf2cejc"},
		player = 0xFF83BE, space = 0x300, 
		offset = {grace = 0x1F0, char_id = 0x291},
		show_OSD = function()
			return bit.band(rw(0xFF8008), 0x08) > 0
			-- return rw(0xFF8008) == 0x0A00
		end,
		show_life = function()
	...
	{	games = {"dstlk"},
		...
		show_OSD = function() return any_true({
			...
			-- rd(0xFF8004) == 0x40000 and rb(0xFF800B) == 0 and rb(0xFF800A) == 0 and bit.band(rw(0xFF8008),0x8) == 0x8,
		}) end,
	},
	{	games = {"nwarr"},
	...
	{	games = {"cybots"},
		...
		show_OSD = function() return any_true({
			...
			rb(0xFF8008) == 0 and rb(0xFF8009) == 0 and bit.band(rb(0xFF800A),0x10) == 0x10,
		}) end,
	},
	{	games = {"sgemf"},
		player = 0xFF8400,
	...
	{	games = {"sfiii3","sfiii3n","sfiii3nr1"},
...
local sf2_limit = {
	...
	["sf2cejc"] = function(p) return 0x1F end,
...
for _, g in ipairs(profile) do
	if string.find(g.games[1], "sf2") then
		...
		g.stun_bar = {
			...
			level = function(p) return rw(p.base + 0x05E), 0x1F end,
			timeout = function(p) return get_state({
	...
	g.cpudevice = g.cpudevice or ":maincpu"
	g.memspace  = g.memspace or "program"
...
local function mem_operator_initial(g)
	-- print(g.cpudevice,g.memspace)
	mem = manager:machine().devices[g.cpudevice].spaces[g.memspace] --  mem = manager:machine().devices[":maincpu"].spaces["program"]

	rb  = function(addr) return mem:read_u8(addr) end
	rw  = function(addr) return mem:read_u16(addr) end
	rd  = function(addr) return mem:read_u32(addr) end
	
	rbs = function(addr) return mem:read_i8(addr) end
	rws = function(addr) return mem:read_i16(addr) end
	rds = function(addr) return mem:read_i32(addr) end
end
...
local function pixel(x1, y1, color, dx, dy)
	s:draw_line(x1 + dx, y1 + dy,x1 + dx+1, y1 + dy+1, color)
end

local function line(x1, y1, x2, y2, color, dx, dy)
	s:draw_line(x1 + dx, y1 + dy, x2 + dx, y2 + dy, color)
end

local function box(x1, y1, x2, y2, color, dx, dy)
	s:draw_box(x1 + dx, y1 + dy, x2 + dx, y2 + dy, color)
end

local draw_stun = function(p)
	local f, s, o = 0xFFF8B000, 0xFFB06000, 0xFF500000 --fill, shade, outline colors
...
local draw_player_objects = function(p)
	...
	if show_numbers then
		for _, text in pairs(p.text) do
			s:draw_text(text.X, text.Y, text.display, text.color or 0xFFFFFFFF)
		end
	end
end
...
local function whatgame()
	...
	for _, module in ipairs(profile) do
		for _, shortname in ipairs(module.games) do
			...
			if emu.romname() == shortname then
				print("showing OSD for: " .. emu.gamename())
				game = module
				for p = 1, game.nplayers do
					player[p] = {}
				end
				mem_operator_initial(game)
				game.initial()
				update_OSD()
				return
			end
